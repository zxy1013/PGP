...
**************************************************/
__device__ static uint64_t load64(const uint8_t x[8]) {
  unsigned int i;
  uint64_t r = 0;
...
**************************************************/
__device__ static void store64(uint8_t x[8], uint64_t u) {
  unsigned int i;
...
**************************************************/
__device__ static void KeccakF1600_StatePermute(uint64_t state[25])
{
        int round;
...
**************************************************/
__device__ static void keccak_init(keccak_state *state)
{
  unsigned int i;
  for(i=0;i<25;i++)
    state->s[i] = 0;
...
**************************************************/
__device__ static unsigned int keccak_absorb(uint64_t s[25],
                                  unsigned int r,
                                  unsigned int pos,
                                  const uint8_t *m,
                                  size_t mlen)
...
**************************************************/
__device__ static void keccak_finalize(uint64_t s[25], unsigned int r, unsigned int pos, uint8_t p)
{
  unsigned int i,j;
...
__device__ static void keccak_squeezeblocks(uint8_t *out,
                                 size_t nblocks,
                                 uint64_t s[25],
                                 unsigned int r)
{
...
* Returns new position pos in current block
**************************************************/
__device__ static unsigned int keccak_squeeze(uint8_t *out,
                                   size_t outlen,
                                   uint64_t s[25],
                                   unsigned int r,
                                   unsigned int pos)
...
__global__ void shake2561(int group,uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
  ...
  if(X < group){
    ...
    shake256_init(&state);
    shake256_absorb(&state, in+o_input, inlen);
    shake256_finalize(&state);
    shake256_squeeze(out+o_output, outlen, &state);
  }
...
__global__ void shake2562(int group,uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
  ...
  //if(X == 0)
    //for(int k = 0;k<outlen;k++) printf(" %d ",out[k]);
}
...
__global__ void shake2563(int group,uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen)
{
  ...
  if(X < group){
    int o_input = (2*SEEDBYTES + 3*CRHBYTES)*X;
    int o_output = (2*SEEDBYTES + 3*CRHBYTES)*X;
    shake256_init(&state);
    shake256_absorb(&state, in+o_input, inlen);
    shake256_finalize(&state);
    shake256_squeeze(out+o_output, outlen, &state);
...
__global__ void shake256_sign1(int group,int step, int *f, unsigned int *n,keccak_state *state,uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen,const uint8_t *in1, size_t inlen1){
    ...
    if(X < group && (f[X]  == step  && n[X] > OMEGA)){
        int o_input = (2*SEEDBYTES + 3*CRHBYTES)*X;
        int o_input1 = X * (inlen1 + CRYPTO_BYTES);
        int o_output = (2*SEEDBYTES + 3*CRHBYTES)*X;
        shake256_init(state+X);
        shake256_absorb(state+X, in + o_input, inlen);
        shake256_absorb(state+X, in1 + o_input1, inlen1);
        shake256_finalize(state+X);
        shake256_squeeze(out+o_output, outlen, state+X);
    } 
}
__global__ void shake256_sign2(int group,int step, int *f, unsigned int *n,keccak_state *state,uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen,const uint8_t *in1, size_t inlen1, size_t mlen){
   ...
   if(X < group && (f[X]  == step  && n[X] > OMEGA)){
        int o_input = (2*SEEDBYTES + 3*CRHBYTES)*X;
        int o_input1 = X * (mlen + CRYPTO_BYTES);
        int o_output = (mlen + CRYPTO_BYTES) *X;
        shake256_init(state+X);
        shake256_absorb(state+X, in + o_input, inlen);
        shake256_absorb(state+X, in1 + o_input1, inlen1);
        shake256_finalize(state+X);
...
__global__ void shake256_sign3(int group,int step, int *f, unsigned int *n,keccak_state *state,uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen,const uint8_t *in1, size_t inlen1){
   ...
   if(X < group && (f[X]  == step  && n[X] > OMEGA)){
        int o_input = inlen * X;
        int o_input1 = (inlen1 + CRYPTO_BYTES) * X;
        int o_output = outlen * X;
        shake256_init(state+X);
        shake256_absorb(state+X, in + o_input, inlen);
        shake256_absorb(state+X, in1 + o_input1, inlen1);
        shake256_finalize(state+X);
   ...
   // if(X == 0) for(int i = 0;i<outlen;i++)printf(" %d ",out[i]);
}
__global__ void shake256_sign4(int group,int step, int *f, unsigned int *n,keccak_state *state,uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen,const uint8_t *in1, size_t inlen1){
   ...
   if(X < group && (f[X]  == step  && n[X] > OMEGA)){
        int o_input = inlen * X;
        int o_input1 = K*POLYW1_PACKEDBYTES * X;
        int o_output = outlen * X;
        shake256_init(state+X);
        shake256_absorb(state+X, in + o_input, inlen);
        shake256_absorb(state+X, in1 + o_input1, inlen1);
