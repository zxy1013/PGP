...
__global__ void poly_ntt1(int group, int step, int *f, unsigned int *n,poly *c){
  ...
  //if(X == 0) for(int k =0 ;k<N;k++)printf(" %d ",(c+1)->coeffs[k]); 
}
/*************************************************
...
**************************************************/
__global__ void poly_challenge(int group, int step, int *f, unsigned int *n,poly *c, const uint8_t *seed, size_t mlen){
  ...
  int X = threadIdx.x + blockIdx.x * blockDim.x;
  if(X < group && (f[X]  == step  && n[X] > OMEGA)){
      int o_input =  X * (mlen + CRYPTO_BYTES);
      shake256_init(&state);
      shake256_absorb(&state, seed+o_input, SEEDBYTES);
      ...
      for(i = N-TAU; i < N; ++i) {
        ...
        (c+X)->coeffs[i] = (c+X)->coeffs[b];
        (c+X)->coeffs[b] = 1 - 2*(signs & 1);
        signs >>= 1;
      }
  }
  //if(X == 0){
  //for(int k =0 ;k<N;k++)printf(" %d ",(c+1)->coeffs[k]);
  //}
}
