...
__device__ int16_t zetas_inv[128] = {
...
__device__ void ntt_p(poly3* r)
{
	...
	//__shared__ uint16_t MEM[];

	//printf("\nNTT: \n");

	//for (int i = 0; i < 256; i++)
	//{
		//printf("%d ", r->coeffs[i].threads[X]);

		//printf("%02X ", r->coeffs[i].threads[X]);
		//if ((i + 1) % 16 == 0)
		//	printf("\n");
	//}
	...
	for (len = 128; len >= 2; len >>= 1)
	{
		for (start = 0; start < 256; start = j + len)
		{
			...
			for (j = start; j < start + len; ++j)
			{
				t = fqmul(zeta, r->coeffs[j + len].threads[X]);
				r->coeffs[j + len].threads[X] = r->coeffs[j].threads[X] - t;
				r->coeffs[j].threads[X] = r->coeffs[j].threads[X] + t;
			}
		}
	}
}
...
* Arguments:   - int16_t r[256]: pointer to input/output vector of elements of Zq
**************************************************/
__device__ void invntt_p(poly3* r)
{
	int X = threadIdx.x + blockIdx.x * blockDim.x;
	...
	for (len = 2; len <= 128; len <<= 1)
	{
		for (start = 0; start < 256; start = j + len)
		{
			...
			for (j = start; j < start + len; ++j)
			{
				t = r->coeffs[j].threads[X];
				r->coeffs[j].threads[X] = barrett_reduce(t + r->coeffs[j + len].threads[X]);
				r->coeffs[j + len].threads[X] = t - r->coeffs[j + len].threads[X];
				r->coeffs[j + len].threads[X] = fqmul(zeta, r->coeffs[j + len].threads[X]);
			}
		}
	}
	...
	for (j = 0; j < 256; ++j)
	{
		r->coeffs[j].threads[X] = fqmul(r->coeffs[j].threads[X], zetas_inv[127]);
	}
}
...
*              -  int16_t a[2]: pointer to the first factor
*              -  int16_t b[2]: pointer to the second factor
...
__device__ void basemul2(threads r[2],  threads a[2],  threads b[2], int16_t zeta) 
{
	...
	r[0].threads[X] = fqmul(a[1].threads[X], b[1].threads[X]);
	r[0].threads[X] = fqmul(r[0].threads[X], zeta);
	r[0].threads[X] += fqmul(a[0].threads[X], b[0].threads[X]);

	r[1].threads[X] = fqmul(a[0].threads[X], b[1].threads[X]);
	r[1].threads[X] += fqmul(a[1].threads[X], b[0].threads[X]);
}
